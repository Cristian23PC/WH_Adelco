...
export class CurrencyHelpers {
  ...
  private static formatMoneyCurrency = function (price: Money) {
    return Intl.NumberFormat(locale, { style: 'currency', currency: price?.currencyCode ?? 'EUR' }).format(
  ...
  static isEqualCurrency: (value1: Money, value2: Money) => boolean = (value1: Money, value2: Money) => {
    return value1.centAmount === value2.centAmount;
  };
}
