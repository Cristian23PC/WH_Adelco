...
const businessUnitKey = 'businessUnitKey';
const mockConfigService = {
  get: (key: string) => {
    if (key === 'custom-object-sequence.sequenceContainer') return sequenceContainer;
    if (key === 'custom-object-sequence.businessUnitKey') return businessUnitKey;
...
describe('SequenceService', () => {
  ...
  describe('getBusinessUnitKey', () => {
    ...
    describe('when customObjectRepository.getByContainerAndKey rejects', () => {
      beforeEach(async () => {
        try {
          await service.getBusinessUnitKey('error');
        } catch (e) {
    ...
    describe('when customObjectRepository.create rejects', () => {
      beforeEach(async () => {
        try {
          await service.getBusinessUnitKey('create-error');
        } catch (e) {
          response = e as CommercetoolsError;
        }
      });
