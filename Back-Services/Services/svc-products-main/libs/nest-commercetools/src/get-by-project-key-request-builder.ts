import { AuthMiddlewareOptions, Client, ClientBuilder, HttpMiddlewareOptions, JsonObject } from '@commercetools/sdk-client-v2';
...
export default (options: NestCommercetoolsModuleOptions): ByProjectKeyRequestBuilder => {
  ...
  const clientBuilder: ClientBuilder = new ClientBuilder().withProjectKey(projectKey).withHttpMiddleware(httpMiddlewareOptions);
  ...
  clientBuilder.withMiddleware(next => {
    return (request, response) => {
      next(request, {
        ...response,
        reject: error => {
          response.reject(<JsonObject<CommercetoolsError>>(<unknown>new CommercetoolsError(error)));
        }
      });
    };
  });
