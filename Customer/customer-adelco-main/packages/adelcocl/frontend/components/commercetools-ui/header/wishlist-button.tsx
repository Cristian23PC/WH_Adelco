...
const WishListButton: React.FC<WishListButtonProps> = ({
  ...
  //i18n messages
  const { formatMessage: formatWishlistMessage } = useFormat({
    name: 'wishlist'
  });
  ...
  return (
    <div className="flow-root pr-3">
      ...
      >
        <HeartIcon
          className="text-primary-400 group-hover:text-primary-500 dark:text-light-100 dark:group-hover:text-light-100 h-6 w-6 shrink-0"
          aria-hidden="true"
        />
        {wishlistItemCount > 0 && (
          <>
            <span className="bg-accent-400 hover:bg-accent-500 absolute -right-1 -top-0 h-4 w-4 rounded-full">
