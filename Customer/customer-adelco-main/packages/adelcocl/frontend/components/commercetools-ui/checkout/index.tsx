...
const Checkout = ({ shippingCountryOptions }: Props) => {
  ...
  const someOutOfStock = !!data?.lineItems?.find(
    (item) => !item.variant.isOnStock
  );
  ...
  const [checkoutData, setCheckoutData] = useState({
    ...
    shippingCountry: '',
    billingAddress:
      account?.addresses.find((address) => address.isDefaultBillingAddress)
        ?.addressId ?? '',
    shippingAddress:
      account?.addresses.find((address) => address.isDefaultShippingAddress)
        ?.addressId ?? '',
    invoiceId: '',
    pay: 'cc'
  });
  ...
  const isCheckoutDataValid = () => {
    ...
    const ccScheme = yup.object().shape({
      ...
      expirationDate: yup
        ...
        .transform((value, originalValue, ctx) => {
          if (typeof originalValue !== 'string' && ctx.isType(value))
            return value;
    ...
    const scheme = yup
      ...
      .concat(
        (loggedIn
          ? loggedInScheme
          : guestScheme) as yup.ObjectSchema<ObjectShape>
      )
      .concat(
        checkoutData.pay === 'cc'
          ? ccScheme
          : (invoiceScheme as yup.ObjectSchema<ObjectShape>)
      );
  ...
  return (
    <main className="lg:flex lg:min-h-full lg:flex-row-reverse lg:overflow-hidden py-10">
      ...
      <section
        ...
        className="sm:px-6 sm:pt-16 lg:px-8 lg:pt-0 lg:pb-24 flex-auto overflow-y-auto px-4 pb-16 pt-12"
      >
            ...
            <CheckoutForm
              submitText={`${formatCheckoutMessage({
                ...
                CurrencyHelpers.addCurrency(
                  data.sum,
                  shippingMethods.data?.[0]?.rates?.[0]?.price || {}
                )
              )}`}
