...
export default function Newsletter({
  ...
  const { formatMessage: formatNewsletterMessage } = useFormat({
    name: 'newsletter'
  });
  ...
  const fallbackDescription = formatNewsletterMessage({
    ...
    defaultMessage:
      'Subscribe to our newsletter to get notified every time we launch a new collection.'
  });
  ...
  const fallbackSuccessMessage = formatNewsletterMessage({
    ...
    defaultMessage:
      'You’re subscribed. We’ll keep you up to date with all things fashion.'
  });
  ...
  return (
    <div className="dark:bg-primary-200 sm:px-6 lg:px-0 mx-auto max-w-7xl rounded-lg bg-gray-100 px-4 py-2">
      {!isConfirm ? (
        <div className="md:py-6 md:px-12 lg:py-9 lg:px-16 xl:flex xl:items-center relative px-6 py-3 text-neutral-700">
          <div className="xl:w-0 xl:flex-1">
            <h2 className="dark:text-light-100 md:text-left mt-2 text-center text-2xl font-bold tracking-[0.010em]">
              {headline || fallbackHeadline}
            </h2>
            <p className="dark:text-light-100 md:text-left mt-3 max-w-2xl text-center text-sm">
              {description || fallbackDescription}
            </p>
          </div>
          <div className="sm:w-full sm:max-w-md xl:mt-0 xl:ml-8 mt-8">
            <form
              action="#"
              className="sm:mx-auto sm:flex sm:max-w-xl xl:px-4 mt-4"
              onSubmit={onSubmit}
            >
              ...
              <input
                ...
                name="email"
                type="email"
                className="focus:ring-offset-accent-400 block w-full rounded-md border border-transparent px-5 py-3 text-sm leading-5 text-gray-600 shadow-sm placeholder:text-gray-500 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2"
                placeholder={inputPlaceholder}
                onChange={onChange}
              />
              <div className="sm:mt-0 sm:ml-3 mt-4">
                <button
                  type="submit"
                  className="bg-accent-400 hover:bg-accent-500 sm:px-11 block w-full rounded-md px-6 py-3 text-base font-bold leading-5 text-white"
                >
                  <Typography>{ctaLabel || fallbackCta}</Typography>
                </button>
              </div>
            </form>
            <p className="dark:text-light-100 xl:px-5 mt-2 max-w-md text-left text-xs text-neutral-600">
        ...
        <div className="relative py-4">
          <div className="sm:text-center">
            ...
            <h2 className="lg:text-3xl mt-2 text-center text-2xl font-bold tracking-[0.010em]">
