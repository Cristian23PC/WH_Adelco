...
const AdyenCheckout = ({ termsLink, cancellationLink, privacyLink }) => {
  ...
  const { formatMessage: formatCheckoutMessage } = useFormat({
    name: 'checkout'
  });
  const containerRef = useRef();
  const [currentStepIndex, setCurrentStepIndex] = useState(0);
  const [disableSubmitButton, setDisableSubmitButton] = useState<boolean>(true);
  const [billingIsSameAsShipping, setBillingIsSameAsShipping] =
    ...
    if (countryBasedShippingRateIndex[data.shippingCountry] == undefined) {
      toast.error(
        formatCheckoutMessage({
          ...
          defaultMessage:
            'Taxes are not defined for this country in commercetools'
        })
      );
  ...
  const submitButtonLabel = [
    ...
    formatMessage({
      id: 'ContinueAndPay',
      defaultMessage: 'Continue and pay'
    })
  ];
  ...
  useEffect(() => {
    ...
    if (
      defaultData &&
      requiredDataIsValid(defaultData, billingIsSameAsShipping)
    ) {
    ...
    if (!currentShippingMethod && cartList?.availableShippingMethods) {
      if (cartList?.shippingInfo) {
        const currentShippingMethod = cartList.availableShippingMethods.find(
          ({ shippingMethodId }) =>
            shippingMethodId == cartList.shippingInfo.shippingMethodId
        );
  ...
  return (
    <div className="md:mt-4 mx-auto max-w-4xl">
      <div>
        <div className="mx-auto py-6">
          <div
            className="md:px-12 relative flex justify-between px-5 py-6 shadow-md"
            id="ProgressStepper"
          >
            <div className="absolute left-0 top-0 flex h-full w-full items-center justify-between px-12 py-6 ">
              <div className="top-2/4 h-2 w-full bg-green-100"></div>
            </div>
            {steps.map(({ name }, index) => (
              <button
                key={index}
                className="relative rounded bg-white p-2"
                onClick={() => changeStep(index)}
              >
                {generateStepTag(index)}
                <div className="2xl:text-base text-center text-xs">{name}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div
        className="lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16"
        ref={containerRef}
      >
