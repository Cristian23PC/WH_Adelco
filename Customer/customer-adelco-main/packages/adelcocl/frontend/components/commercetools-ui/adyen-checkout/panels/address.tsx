...
const Address: React.FC<AddressProps> = ({
  ...
  const [projectSettingsCountries, setProjectSettingsCountries] =
    useState<ProjectSettings>(null);
  const [shippingMethodsData, setShippingMethodsData] =
    useState<ShippingMethod[]>(null);
  const [availableCountryOptions, setAvailableCountryOptions] =
    useState<CountryOption[]>(null);
  const { getProjectSettings, shippingMethods } = useCart();
  const { formatMessage } = useFormat({ name: 'checkout' });
  const { formatMessage: formatCommonMessage } = useFormat({
    name: 'common'
  });
    ...
    if (!shippingMethods.data || !projectSettingsCountries) {
      const showMessageInDropdown = {
        ...
        display: `${formatMessage({
          ...
          defaultMessage:
            'Currently there are no countries available for shipping'
        })}`
      };
      setAvailableCountryOptions([showMessageInDropdown]);
    } else {
      const totalCountries = getTaxedCountries(
        shippingMethods?.data,
        projectSettingsCountries?.countries
      );
  ...
  return (
    <section
      ...
      className="md:rounded md:shadow-md lg:col-span-7 bg-white"
    >
      <form className="md:border-t-0 md:px-6 border-t-4 border-neutral-100 bg-white px-4 py-5">
        ...
        <div className="mb-4">
          ...
          <input
            className=" w-full appearance-none rounded border border-gray-300 px-3 py-2 leading-tight text-gray-700 shadow focus:outline-none"
            id="firstName"
            name="firstName"
            type="text"
        ...
        <div>
          <label
            className="text-sm leading-tight text-neutral-700"
