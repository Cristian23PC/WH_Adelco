...
export default function ProductDetail({
  ...
  useEffect(() => {
    const idx = product?.variants.findIndex(
      (v: Variant) =>
        v.attributes.color?.key === selectedColor?.key &&
        v.attributes.commonSize?.key === selectedSize?.key
    );
  ...
  return (
    <div>
      <div className=" md:py-4 lg:max-w-7xl lg:px-8 mx-auto max-w-2xl">
        <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
          {/* Image gallery */}
          <Tab.Group>
            <div className="flex flex-col-reverse">
              {/* Image selector */}
              <div className="sm:block lg:max-w-none mx-auto mt-6 hidden w-full max-w-2xl">
                ...
                {product?.images?.map((image) => (
                  <Tab.Panel key={image.id}>
                    <Image
                      ...
                      src={image.src}
                      alt={image.alt}
                      className="sm:rounded-lg w-full object-cover object-center"
                    />
                  </Tab.Panel>
                ))}
              </Tab.Panels>
          ...
          {/* Product info */}
          <div className="sm:mt-16 sm:px-0 lg:mt-0 mt-10 px-4">
            ...
            <div className="mt-3">
              ...
              <p className="text-accent-400 text-3xl">
            ...
            <form className="mt-6">
                ...
                <div className="mt-8">
                  ...
                  >
                    ...
                    <div className="sm:grid-cols-6 grid grid-cols-3 gap-3">
                      {product?.sizes?.map(
                        (size: { label: string; key: string }) => (
                          <RadioGroup.Option
                            key={size.label}
                            value={size}
                            className={({ active, checked }) =>
                              classNames(
                                active || selectedSize?.key == size.key
                                  ? 'ring-accent-400 ring-2 ring-offset-2'
                                  : '',
                                checked
                                  ? 'dark:text-light-100 bg-transparent text-gray-900 hover:bg-gray-50'
                                  : 'border-gray-200 bg-white text-gray-900 hover:bg-gray-50',
                                'sm:flex-1 flex cursor-pointer items-center justify-center rounded-md border px-3 py-3 text-sm font-medium uppercase'
                              )
                            }
                          >
                            <RadioGroup.Label>
              ...
              <div className="sm:flex-1 mt-10 flex">
                <button
                  ...
                  className="bg-accent-400 hover:bg-accent-500 focus:bg-accent-500 focus:ring-accent-400 flex w-full flex-1 items-center justify-center rounded-md border border-transparent px-8 py-3 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 disabled:bg-gray-400"
                    ...
                    <Disclosure key={detail.name}>
                      {({ open }) => (
                        <>
                            ...
                            <Disclosure.Button className="group relative flex w-full items-center justify-between py-6 text-left">
                              <span
                                className={classNames(
                                  open
                                    ? 'text-accent-400'
                                    : 'dark:text-light-100 text-gray-900',
                                  'text-sm font-medium'
                                )}
                              >
                                {detail.name}
                          ...
                          <Disclosure.Panel>
                            <div className="prose prose-sm dark:text-light-100 py-6">
                              <ul role="list">
