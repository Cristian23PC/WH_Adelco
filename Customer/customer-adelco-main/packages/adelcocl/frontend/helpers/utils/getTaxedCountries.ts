...
export const getTaxedCountries = (
  shippingMethods?: ShippingMethod[],
  projectSettingsCountries?: string[]
) => {
  ...
  shippingMethods?.forEach((shippingMethod) => {
    shippingMethod?.rates?.forEach((rate) => {
      rate?.locations?.forEach((location) => {
        const listOfCountryOptions = countryOptions.map(
          (country) => country.data
        );
        if (
          listOfCountryOptions.includes(location.country) &&
          !taxedCountries.includes(location.country)
        ) {
  ...
  const availableCountries = findDuplicates([
    ...taxedCountries,
    ...projectSettingsCountries
  ]).sort();

  //find country options that match available countries
  return countryOptions.filter(
    (country) => availableCountries?.indexOf(country.data) !== -1
  );
};
