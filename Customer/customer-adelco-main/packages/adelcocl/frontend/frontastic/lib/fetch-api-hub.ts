...
import { KEY as USE_LOGOUT_KEY } from 'frontastic/actions/adelco/user/useLogout/useLogout';
...
export const rawFetchApiHub: FetchFunction = async (
  endpointPath,
  init = {},
  payload = null
) => {
  return await performFetchApiHub(endpointPath, init, payload, {
    getCookie: (cookieIdenfier) => {
      ...
      return window.localStorage.getItem(cookieIdenfier);
    },
    setCookie: (cookieIdenfier, cookieValue) => {
      ...
      //   expires: expiryDate
      // });
      window.localStorage.setItem(cookieIdenfier, cookieValue);
    }
  });
};
  ...
  return fetchApiHubPromise
    ...
    .then(async (response) => {
      if (response.error || response.ok === false) {
        if (response.error?.expiredRefresh) {
          await fetchApiHub(USE_LOGOUT_KEY, { method: 'POST' });
          location.href = '/?openLogin=true&expiredRefresh=true';
        } else {
          throw response;
        }
      }
...
export const fetchApiHubServerSide = async (
  ...
  return handleApiHubResponse(
    rawFetchApiHubServerSide(endpointPath, expressMessages, headers)
  );
};
